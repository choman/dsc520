...
universe (e.g. Marvel). Below is the top ten by vote movies:
...
`r round(topmovie_rt_ave, digits=2)` minutes (`r round(topmovie_rt_ave / 60, digits=1)` hours) per film beat the `r secondplace_rt_count` movie `r secondplace_rt` with 
...
*NOTE*: This however is straight movie collections and not total collections by 
universe (e.g. Marvel). Below is the top ten by runtime total:
...
`r round(topmovie_ave_ave, digits=2)` minutes (`r round(topmovie_ave_ave / 60, digits=1)` hours) per film beat the `r secondplace_ave_count` movie `r secondplace_ave` with 
a total collection runtime of `r secondplace_ave_min` minutes 
...
*NOTE*: This however is straight movie collections and not total collections by 
universe (e.g. Marvel). Below is the top ten by runtime ave:
...
changing to the search parameter date of `r` since`. Since the collections
are not all inclusive, it also makes determining the most popular/best movie 
collection not accurate.
...
I feel that possibly bar charts may help explain the data better. However, the 
knitr::ktables() look good. 

I finally figured out how to do embedded R objects, which I believe help make
this RMarkdown file more dynamic when generating the PDF.
